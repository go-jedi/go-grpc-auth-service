...
func TestGetRefreshToken(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			authServiceMockFunc: func(mc *gomock.Controller) service.AuthService {
				...
				mock.EXPECT().GetRefreshToken(ctx, refreshToken).Return(expected, nil)
				return mock
			},
		},
		{
			...
			authServiceMockFunc: func(mc *gomock.Controller) service.AuthService {
				mock := serviceMocks.NewMockAuthService(mc)
				mock.EXPECT().GetRefreshToken(ctx, refreshToken).Return("", serviceErr)
				return mock
			},
		},
	}
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			result, err := authServiceMock.GetRefreshToken(test.input.ctx, test.input.refreshToken)
